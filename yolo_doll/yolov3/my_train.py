...
def _main():
    train_path = '../data/final_train.txt' #待訓練清單(YOLO格式）
    val_path = '../data/final_val.txt'
    log_dir = '../logs/000/' #訓練過程及結果暫存路徑
    classes_path = '../data/my_classes.txt' #自定義標籤檔路徑及名稱
    anchors_path = '../data/yolo_anchors.txt' #錨點定義檔路徑及名稱
    class_names = get_classes(classes_path)
    num_classes = len(class_names)
    anchors = get_anchors(anchors_path)

    input_shape = (32*13,32*13) # multiple of 32, hw 預設輸入影像尺寸須為32的倍數(寬，高)

    is_tiny_version = len(anchors)==6 # default setting
    if is_tiny_version:
        model = create_tiny_model(input_shape, anchors, num_classes,
            freeze_body=2, weights_path='../data/tiny_yolo_weight.h5')
    else:
        model = create_model(input_shape, anchors, num_classes,
            freeze_body=2, weights_path='../data/trained_weights_final.h5') #指定起始訓練權重檔路徑及名稱
    ...
    if True:
        ...
        batch_size = 12 #批次處理數量，依GPU記憶體大小決定
...
def create_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
            weights_path='../data/yolo_weight.h5'):
...
def create_tiny_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
            weights_path='../data/tiny_yolo_weight.h5'):
